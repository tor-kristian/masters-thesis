...
function SympHadesEnc(State,d,rounds,key,t,c,Keey)
    ...
    for i in [rfstart+1..rfstart+rp] do
        ...
        State := AddKey(State,key);
        State := PartialSbox(State,d);
        State := MultM(State,MRing);

        if rf ge 4 then 
            if i ne (rfstart+rp) then
                I := I cat [State[ind,1]-R.count];
                State[ind,1] := R.count;
                count := count+1;
            end if;
    ...
    for i in [rfstart+rp+1..r-1] do
        ...
        if Degree(State[2,1]) ge 9 then
            I,State,count := addToIdeal(I,State,count); // Avoid higher than degree d^d^d polynomials
        end if;
    ...
    pots := [];
    for i in [1..#J] do
        if IsUnivariate(J[i]) and Degree(J[i]) ge 2 then
            for j in [1..p] do	
                I2 := I;
                k := LeadingMonomial(J[i]);
                
                J2 := GroebnerBasis(I2 cat [k+j],3);			
                if J2 ne [1] then
                    pots := pots cat [J2[#J2]];	
                end if;
            end for;
